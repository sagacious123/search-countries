    ...
    <div className="mb-14">
      <form className="flex justify-between items-start sm:items-center flex-col sm:flex-row sm:max-w-7xl w-11/12 m-auto">
        ...
        <div className=" w-3/5 sm:w-3/12 lg:w-2/12 relative">
          ...
          <input type="button" onClick={handleClick} value={regionDisplayValue} className={`dropdown bg-white dark:bg-darkbg mb-1 text-left text-sm p-3 placeholder-gray-300 px-4 rounded-md shadow-md w-full cursor-pointer ${click && "ring-1 ring-gray-300"}`} />
          { click &&
          ...
          <i className={`fa fa-chevron-down absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-600 transition-all duration-300 ${click && "transform rotate-180"}`}></i>
        </div>
      </form>
    </div>
  )
